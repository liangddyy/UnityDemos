	...
	void OnGUI(){
		...
		if(!dead && !weaponSheathed){
			...
			if(!blockGui && !isBlocking && !ledge){
				...
				if(warrior == Warrior.Sorceress || warrior == Warrior.Karate){
				...
				if(!blockGui && !isBlocking){
					...
					if(warrior == Warrior.Ninja || warrior == Warrior.Crossbow || warrior == Warrior.Karate){
					...
					if(warrior == Warrior.Archer || warrior == Warrior.Sorceress || warrior == Warrior.Karate){
					...
					if(warrior == Warrior.Ninja || warrior == Warrior.Sorceress || warrior == Warrior.Karate || warrior == Warrior.Knight){
						if(GUI.Button(new Rect(135, 175, 100, 30), "SpecialAttack2")){
							if(attack == 0){
								if(warrior == Warrior.Sorceress){
									...
									animator.SetTrigger("SpecialAttack2Trigger");
									if(warrior == Warrior.Knight){
										StartCoroutine(_LockMovementAndAttack(.85f));
									}
									else{
										StartCoroutine(_LockMovementAndAttack(1.25f));
									}
